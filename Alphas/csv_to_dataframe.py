...
def create_y(data, alphas_df):
    closed_shifted = data["Close"].shift(
        -39
    )  # shifted up 39 days to get the closing price 39 days from now
    closed_shifted = closed_shifted[:-39]
    closed = data["Close"][
        :-39
    ]  # making sure that both columns of data are the same length
...
def make_weights(ticker, start_date="2014-01-01", end_date="2024-01-01"):
    ...
    y_pred, accuracy, weights, error = neural_net.linear_regression(
...
def main():
    tickers = ["AAPL", "RL", "TSLA", "DIS", "MSFT", "NFLX", "AMZN", "GOOGL"]
